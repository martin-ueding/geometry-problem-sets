...
$\dif t$, else it is mapped to zero.
...
For $\psi$ we know that there is a $\dif t$ term and that this will cancel.
$\dif \psi$ therefore is simpler than $\dif \omega$:
\[
    \dif \psi = a_{,l}(\tens x) \dif x^l \wedge
    \bigwedge_{n = 1}^{p} \dif x^{i_n}.
\]
